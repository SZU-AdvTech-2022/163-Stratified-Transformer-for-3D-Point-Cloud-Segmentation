...
def get_parser():
    ...
    cfg = config.load_cfg_from_cfg_file(args.config)    # 读取配置文件
...
def main():
    ...
    if args.manual_seed is not None:    # 固定生成
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if main_process():
        ...
        if args.get("max_grad_norm", None): # 未找到
    ...
    if main_process():
        logger.info("ddddebug:{}".format(args.resume))
