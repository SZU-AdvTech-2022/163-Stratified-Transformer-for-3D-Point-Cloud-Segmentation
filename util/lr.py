...
class PolyLRwithWarmup(LambdaStepLR):
    ...
    def __init__(self, optimizer, max_iter, warmup='linear', warmup_iters=1500, warmup_ratio=1e-6, power=1.0, last_step=-1):
        ...
        super(PolyLRwithWarmup, self).__init__(
            optimizer, poly_with_warmup, last_step)
        # torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda, last_epoch=-1, verbose=False)
        # lr_lambda: A function which computes a multiplicative factor given an integer parameter epoch, or a list of such functions, 
...
class PolyLR(LambdaStepLR):
    ...
    def __init__(self, optimizer, max_iter, power=0.9, last_step=-1): 
        super(PolyLR, self).__init__(optimizer, lambda s: (
            1 - s / (max_iter + 1))**power, last_step)
        # torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda, last_epoch=-1, verbose=False)
        # lr_lambda: A function which computes a multiplicative factor given an integer parameter epoch, 
...
def initialize_scheduler(optimizer, config, last_epoch=-1, scheduler_epoch=True, logger=None):
    ...
    if config.scheduler == 'StepLR':
        ...
        return PolyLRwithWarmup(optimizer, max_iter=config.max_iter, warmup=config.warmup, warmup_iters=config.warmup_iters, \
          warmup_ratio=config.warmup_ratio, power=config.poly_power, last_step=last_step)
    ...
    # (optimizer, max_lr, total_steps=None, epochs=None, steps_per_epoch=None, pct_start=0.3, anneal_strategy='cos', \
    # cycle_momentum=True, base_momentum=0.85, max_momentum=0.95, div_factor=25.0, final_div_factor=10000.0, last_epoch=-1)
    else:
